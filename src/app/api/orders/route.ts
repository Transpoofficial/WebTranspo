...
const validateTransportPricing = async (
    ...
    for (const [dateStr, tripDestinations] of tripsByDate.entries()) {
        ...
        try {
            ...
            console.error(
                `Error calculating distance for trip ${dateStr}:`,
                error
            );
    ...
    if (
        ...
        logCalculationDiscrepancy({
            ...
            distanceDifference: Math.abs(
                actualTotalDistanceKm - frontendDistanceKm
            ),
            priceDifference,
...
const handleTransportOrder = async (
    ...
    if (requiresAllDestinationRestriction(vehicleType.name)) {
        ...
        const uniqueDates = new Set(
            destinations.map((dest) => dest.departureDate)
        );
...
export const GET = async (req: NextRequest) => {
    try {
        ...
        if (dateFilter === "custom" && startDate && endDate) {
            ...
            // console.log(
            //     `Predefined date filter applied: last ${dateFilter} days`
            // );
...
export const POST = async (req: NextRequest) => {
    try {
            ...
            if (destMatch) {
                ...
                switch (property) {
                    ...
                    case "isPickupLocation":
                        ...
                        break;
                    case "sequence":
                        destinations[index].sequence = parseInt(
                            value as string,
                            10
                        );
                        break;
        ...
        if (
            orderType?.toUpperCase() === "TRANSPORT" &&
        ...
        if (!orderType || !timezone) {
            return NextResponse.json(
                {
                    message: "Missing required fields: orderType or timezone",
                    data: [],
                },
                { status: 400 }
            );
            ...
            async (tx) => {
                ...
                const createdOrder = await tx.order.create({
                    data: {
                        ...
                        fullName:
                            fullName || userExists?.fullName || "Customer",
                        phoneNumber:
                            phoneNumber || userExists.phoneNumber || "",
                        email: email || userExists.email || "",
                        totalPassengers: totalPassengers
                            ? parseInt(totalPassengers)
                            : null,
